...
function showTotalNumberDashboard(loggedInUserId) {
    $.ajax({
        type: 'GET',
        url: 'https://meditation-tracker.herokuapp.com/sessions-total/' + loggedInUserId,
    })
    .done(function (res) {
...
function showStreakDashboard(loggedInUserId) {
    $.ajax({
        type: 'GET',
        url: 'https://meditation-tracker.herokuapp.com/sessions-streak/' + loggedInUserId,
    })
    .done(function (res) {
...
function showLastTenDaysDashboard(loggedInUserId) {
    $.ajax({
        type: 'GET',
        url: 'https://meditation-tracker.herokuapp.com/sessions-ten/' + loggedInUserId,
    })
    .done(function (res) {
        let sessionDays = res.map(a => a.sessionDateUnix);
...
function showMethodUsedDashboard(loggedInUserId) {
    $.ajax({
        type: 'GET',
        url: 'https://meditation-tracker.herokuapp.com/sessions-method/' + loggedInUserId,
    })
    .done(function (res) {
...
function showAvgLengthDashboard(loggedInUserId) {
    $.ajax({
        type: 'GET',
        url: 'https://meditation-tracker.herokuapp.com/sessions-avg/' + loggedInUserId,
    })
    .done(function (res) {
...
function showJournalScreen(loggedInUserId) {
	...
	$.ajax({
		type: 'GET',
		url: 'https://meditation-tracker.herokuapp.com/sessions-journal/' + loggedInUserId,
	})
	.done(function (res) {
...
function checkDuplicateEmail(inputEmail) {
    $.ajax({
        type: 'GET',
        url: `https://meditation-tracker.herokuapp.com/check-duplicate-email/${inputEmail}`,
        dataType: 'json',
...
$(document).on('click', '#js-signup-button', function(event) {
		...
		if (password !== confirmPw) {
			...
			$.ajax({
				type: 'POST',
				url: 'https://meditation-tracker.herokuapp.com/users/create',
				dataType: 'json',
...
$(document).on('click', '#js-login-button', function(event) {
    ...
    if ((!inputEmail) || (inputEmail.length < 1) || (inputEmail.indexOf(' ') > 0)) {
        ...
        $.ajax({
                type: 'POST',
                url: 'https://meditation-tracker.herokuapp.com/signin',
                dataType: 'json',
...
$(document).on('click', '#js-save-session', function(event) {
    ...
    if (sessionDate == "") {
		...
		$.ajax({
			type: 'POST',
			url: 'https://meditation-tracker.herokuapp.com/sessions/create',
			dataType: 'json',
...
$(document).on('submit', '#changePw-form', function(event) {
    ...
    const pw2 = $('input[name="js-confirm-userPw"]').val();
        if (pw !== pw2) {
            ...
            $.ajax({
                type: 'PUT',
                url: 'https://meditation-tracker.herokuapp.com/user-pw/' + loggedInUserId,
                dataType: 'json',
